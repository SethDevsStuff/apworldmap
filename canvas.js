...
function initCanvas(id, settings){//ID of HTML element you want canvas to be in. Better to use <div> or <custom-element>
    ...
    canvas.element.style["background-color"] = settings.color || "silver";
...
function Component(x, y, size, settings){
    ...
    this.getOpacity = function () {
        if(this.group.x != undefined){
            return this.opacity * this.group.getOpacity();
        }
        return this.opacity;
    }
    ...
    this.addAnimation = function (name, changes, timeFrame, animationType, runAfter) {
        var animation = {
            changes,
            timeFrame,
